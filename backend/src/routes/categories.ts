...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res.json(await loadCategories({ kind, type, includeMembers }));
  } catch (error) {
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      res.status(201).json(await loadCategoryById(categoryId));
    } catch (error) {
...
router.put('/:id', validateId, async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      await connection.commit();
      res.json(await loadCategoryById(id));
    } catch (error) {
      await connection.rollback();
      throw error;
    } finally {
...
router.delete('/:id', validateId, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [budgetRows] = await pool.execute('SELECT COUNT(*) AS count FROM budgets WHERE category_id = ?', [id]);
    if (Number((budgetRows as any[])[0]?.count || 0) > 0) {
      return res.status(400).json({ error: '该分类已被预算使用，无法删除' });
    }

    if (category.kind === 'leaf') {
      const [transactionRows] = await pool.execute('SELECT COUNT(*) AS count FROM transactions WHERE category_id = ?', [id]);
      if (Number((transactionRows as any[])[0]?.count || 0) > 0) {
        return res.status(400).json({ error: '该分类有关联的交易记录，无法删除' });
      }

      const [groupRows] = await pool.execute(
        'SELECT COUNT(*) AS count FROM category_group_members WHERE member_category_id = ?',
        [id],
      );
      if (Number((groupRows as any[])[0]?.count || 0) > 0) {
