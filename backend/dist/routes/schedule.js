...
function numberListsOverlap(left, right) {
...
async function findConflictingCourse(dayOfWeek, timeSlot, weeks, excludeId) {
...
router.get('/', async (req, res) => {
    try {
        const [rows] = await database_1.default.execute('SELECT * FROM schedule_courses ORDER BY day_of_week, time_slot');
        const courses = rows.map(normalizeStoredCourse);
        res.json(courses);
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const [result] = await database_1.default.execute(`INSERT INTO schedule_courses (name, teacher, location, color, day_of_week, time_slot, weeks)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [data.name, data.teacher, data.location, data.color, data.dayOfWeek, timeSlotJson, weeksJson]);
        res.status(201).json({
            id: result.insertId,
            name: data.name,
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        const timeSlotJson = JSON.stringify(data.timeSlot);
        const weeksJson = JSON.stringify(data.weeks);
        const [result] = await database_1.default.execute('UPDATE schedule_courses SET name = ?, teacher = ?, location = ?, color = ?, day_of_week = ?, time_slot = ?, weeks = ? WHERE id = ?', [data.name, data.teacher, data.location, data.color, data.dayOfWeek, timeSlotJson, weeksJson, courseId]);
...
router.delete('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        await database_1.default.execute('DELETE FROM schedule_courses WHERE id = ?', [id]);
        res.json({ success: true });
    }
    catch (error) {
...
router.post('/import', async (req, res) => {
    ...
    for (const [index, course] of courses.entries()) {
        ...
        const conflictingCourse = normalizedCourses.find((existingCourse) => (existingCourse.dayOfWeek === data.dayOfWeek && coursesConflict(existingCourse, data)));
    ...
    try {
        ...
        for (const course of normalizedCourses) {
            await connection.execute('INSERT INTO schedule_courses (name, teacher, location, color, day_of_week, time_slot, weeks) VALUES (?, ?, ?, ?, ?, ?, ?)', [course.name, course.teacher, course.location, course.color, course.dayOfWeek, JSON.stringify(course.timeSlot), JSON.stringify(course.weeks)]);
        }
