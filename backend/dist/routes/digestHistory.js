...
router.get('/', async (req, res) => {
    try {
        ...
        const digestType = digestTypeInput ? ((0, digestHistory_1.isDigestType)(digestTypeInput) ? digestTypeInput : null) : undefined;
        if (digestType === null) {
        ...
        const result = await (0, digestHistory_1.listDigestHistory)({
            digestType,
            query,
            days,
            page,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const deleted = await (0, digestHistory_1.deleteDigestHistoryById)(id);
        if (!deleted) {
...
router.post('/clear', async (req, res) => {
    try {
        ...
        const deletedCount = await (0, digestHistory_1.clearDigestHistory)({
            digestType,
            days,
            query,
        });
